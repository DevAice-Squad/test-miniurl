...
app.use(cors({
  origin: function(origin, callback) {
    ...
    const allowedOrigins = [
      ...
      'http://localhost:3000',
      'http://localhost:3001',
      'http://miniurl-alb-701698051.us-east-1.elb.amazonaws.com',
...
app.use((error, req, res, next) => {
  ...
  // URI decoding errors
  if (error instanceof URIError || error.message.includes('Failed to decode param')) {
    console.warn(`URI decoding error from IP ${req.clientIp}: ${error.message}`);
    return res.status(400).json({
